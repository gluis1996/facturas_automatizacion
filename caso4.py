...
def extraer_valor_por_patron(text, patron):
...
# Patrones de expresiones regulares para extraer valores
patron_prima = r'Prima\s*[:]?[\s]*([\d\.,]+)'
patron_igv = r'I\.G\.V\.\s*[:]?[\s]*([\d\.,]+)|IGV\s*[:]?[\s]*([\d\.,]+)'
patron_importe_total = r'Importe\s*Total\s*[:]?[\s]*([\d\.,]+)'
                    ...
                    with pdfplumber.open(pdf_path) as pdf:
                        ...
                        for page in pdf.pages:
                            ...
                            if text:
                                ...
                                if prima is None:
                                    prima = extraer_valor_por_patron(text, patron_prima)

                                if igv is None:
                                    igv = extraer_valor_por_patron(text, patron_igv)

                                if importe_total is None:
                                    importe_total = extraer_valor_por_patron(text, patron_importe_total)
