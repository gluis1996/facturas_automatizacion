    ...
    for key, pattern in plantilla.items():
        if key != "identificador":  # Saltar el identificador
            ...
            if match:
                ...
                if valor:  # Solo procede si valor no es None
                    print(f"Match encontrado para {key}: {valor}")
                    try:
                        datos_extraidos[key] = valor.replace(',', '.').strip()
                    except AttributeError as e:
                        print(f"Error al procesar el valor para {key}: {e}")
                        datos_extraidos[key] = None
                else:
                    print(f"No se encontró un valor válido para {key}")
                    datos_extraidos[key] = None
            else:
